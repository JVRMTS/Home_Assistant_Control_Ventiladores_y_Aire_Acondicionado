...
// Función para configurar el reloj con la zona horaria y el horario de verano para Europa Central.
void configReloj(){
  // Confinguramos el servidor NTP
  const char* ntpServer = "pool.ntp.org";                   // Servidor NTP para sincronizar el reloj
  const long gmtOffset_sec = 3600;                          // Selección de la zona horaria GMT+1 para España
  const int daylightOffset_sec = 3600;                      // Configuración para el horario de verano
  String timezone = "CET-1CEST,M3.5.0,M10.5.0/3";           // TimeZone, regla para Europa/Madrid incluyendo el horario de verano
  configTime(gmtOffset_sec, daylightOffset_sec, ntpServer); // Configuramos la hora a través del servidor NTP
  setTimezone(timezone);                                    // Establecemos la zona horaria
}
